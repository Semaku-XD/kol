...
def menu(my_name,my_id):
	...
	print(f'{m}[+]ID Tumbal: '+str(my_id))
	print(f'{k}[+]Ip Mu  : {ip}')
	print(f'{h}[+]Github   : {gh}')
	cetak(nel('\t[bold cyan]            • SILAKAN PILIH MAU NGAPAIN • [/bold cyan]'))
	print('')
...
def dump_massal():
	...
	try:
		...
		print('{k}➣ Masukkan Angka Anjing, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		print('')
		print(f'{k}➣ Total Idz Yang Terkumpul★{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{k}')
...
#			id2.insert(xx,bacot)
		...
		exit()
	print(f' {sir}{b}                           SILAKAN TEKAN 1 UNTUK LANJUT ')
#	print('2. Mbasic ')
#	print('3. Touch  ')
#	print('4. Mtouch ')
	print('')
...
def passwrd():
	...
	if woi in ['y','Y']:
		...
		print(f'\t{u} SALAM DARI SAYA (POPEN NOPRIANSYAH')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{k}忠SALAM KENAL DARI SAYA\n\t{k}忠 POPEN NOPRIANSYAH\n\t{k}忠INI SCRIPT GRATIS, TIDAKDIPERJUAL BELIKAN\n\t{k}忠{u}GUNAWKAH TERSERAH KABA \n\t{k}忠{m}AMAW ADAW BUG/EROR NGICIAK AWU\n\t{k}忠{k}DOA KA AJAW AKU SEHAT \n\t{k}忠{h}DAPAT BINI SOLEHA\n\t{k}忠{b}AMINKAH MANGKU KAMU DAPAT PAHALA \n\t{k}忠{p}JANGAN PACAK COLI TULA\n\t{k}忠{hh}KALU DAPAT AZAB,INGATKA AWAK TU LUM NIKAH\n\t{k}忠{u}CUMAN NGICIAK KA NYAW AJAW\n\t{k}忠{h}KALU GI KEPIKIRAN NDAK NIKAH\n\t{k}忠{k}TAPI AMAW NDIK GILA\n\t{k}忠{m}TERSERAH KABA LA\n\t{k}忠{b}ASAK KA KABA SENANG\n\t{k}忠{h}KURANG SENANG TEMUI AKU\n\t{k}忠{b}DI CINTO MANDI\n\t{k}忠{u}NDIK KRUAN CINTO MANDI? \n\t{k}忠{p}KINAK I GOOGLE MAP\n\t{k}忠{h}JANGAN BIGAL NIAN\n\t{k}忠{u}LA BESAK KABA TU{x}')
	time.sleep(3)
	login()
